...
class ModeratorSupplierRepository:
    ...
    async def batch_list_domains(self, supplier_ids: List[int]) -> dict[int, List[str]]:
        if not supplier_ids:
            return {}
        result = await self.session.execute(
            select(SupplierDomainModel.supplier_id, SupplierDomainModel.domain)
            .where(SupplierDomainModel.supplier_id.in_(supplier_ids))
            .order_by(SupplierDomainModel.supplier_id, SupplierDomainModel.is_primary.desc(), SupplierDomainModel.domain.asc())
        )
        out: dict[int, List[str]] = {sid: [] for sid in supplier_ids}
        for row in (result.fetchall() or []):
            if row and row[0] is not None and row[1]:
                out.setdefault(int(row[0]), []).append(str(row[1]))
        return out

    async def batch_list_emails(self, supplier_ids: List[int]) -> dict[int, List[str]]:
        if not supplier_ids:
            return {}
        result = await self.session.execute(
            select(SupplierEmailModel.supplier_id, SupplierEmailModel.email)
            .where(SupplierEmailModel.supplier_id.in_(supplier_ids))
            .order_by(SupplierEmailModel.supplier_id, SupplierEmailModel.is_primary.desc(), SupplierEmailModel.email.asc())
        )
        out: dict[int, List[str]] = {sid: [] for sid in supplier_ids}
        for row in (result.fetchall() or []):
            if row and row[0] is not None and row[1]:
    ...
    async def list(
        ...
        if search and search.strip():
            ...
            from app.adapters.db.models import SupplierDomainModel
            search_filter = or_(
                func.lower(ModeratorSupplierModel.name).like(s),
                func.lower(ModeratorSupplierModel.inn).like(s),
                func.lower(ModeratorSupplierModel.domain).like(s),
                ModeratorSupplierModel.id.in_(
                    select(SupplierDomainModel.supplier_id).where(
                        func.lower(SupplierDomainModel.domain).like(s)
                    )
                ),
            )
...
class ParsingRequestRepository:
    ...
    async def get_by_id(self, request_id: int) -> Optional[ParsingRequestModel]:
        """Get parsing request by ID."""
        result = await self.session.execute(
            select(ParsingRequestModel).where(ParsingRequestModel.id == request_id)
        )
        return result.scalar_one_or_none()

    async def update(self, request_id: int, patch: dict) -> Optional[ParsingRequestModel]:
        """Update parsing request fields."""
        request = await self.get_by_id(request_id)
        if not request:
            return None

        valid_fields = {
            'raw_keys_json', 'depth', 'source', 'comment', 'title', 'created_by'
        }
        for key, value in patch.items():
            if key in valid_fields and hasattr(request, key):
...
class ParsingRunRepository:
    ...
    async def list(
        ...
        try:
            ...
            # Legacy schema fallback: parsing_runs table may not have request_id yet
            msg = str(getattr(e, "orig", e))
            if "parsing_runs.request_id" not in msg and "column parsing_runs.request_id" not in msg and "request_id" not in msg:
                raise

            # Ensure session is usable after a failed statement (PostgreSQL aborts the transaction)
            try:
            ...
            sql = text(
                f"""
                SELECT run_id, keyword, status, started_at, finished_at, error, results_count, created_at
                FROM parsing_runs
                {where_sql}
            ...
            sql_result = await self.session.execute(sql, params)
            ...
            for row in rows:
                ...
                run.keyword = row[1]
                run.status = row[2]
                run.started_at = row[3]
                run.finished_at = row[4]
                run.error_message = row[5]
                run.results_count = row[6]
                run.created_at = row[7]
                run.depth = None
                run.source = None
                run.request = None
