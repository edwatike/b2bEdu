...
def extract_item_names_via_groq_with_usage(*, text: str, api_key: str, max_retries: int = 3) -> tuple[List[str], dict]:
    ...
    system = (
        "Ты извлекаешь ТОЛЬКО КОРОТКИЕ наименования позиций (товаров/работ) из текста документа. "
        "Верни ТОЛЬКО JSON без markdown и без пояснений. "
        "Схема строго: {\"names\": [string, ...]}. "
        "Каждый элемент names — одно наименование: без номера строки, без кол-ва, без единиц измерения, без цены/суммы/НДС/итого. "
        "Запрещено возвращать табличные строки целиком. Запрещено включать телефоны, email, URL, ИНН/КПП. "
        "Запрещено включать технические параметры и хвосты спецификаций: DN/DNID/SN/PN/SDR/Ø, проценты, мм/м, L=, D=, толщины, веса, шаги, артикулы-колонки. "
        "Цифры допускаются только если они НЕОТЪЕМЛЕМАЯ часть названия/маркировки (например 33*33, 1000x1000, A60, Zn). "
        "Не добавляй позиции, которых нет в тексте."
    )
