...
async def get_moderator_dashboard_stats(
    ...
    q = await db.execute(
        text(
            ...
            f"  SELECT DISTINCT {s_expr} AS d FROM moderator_suppliers s WHERE s.domain IS NOT NULL"
            "  UNION "
            f"  SELECT DISTINCT {sd_expr} AS d FROM supplier_domains sd WHERE sd.domain IS NOT NULL"
            "),"
            "pending AS ("
