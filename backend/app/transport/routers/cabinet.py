...
async def _ensure_request_suppliers_loaded(*, db: AsyncSession, request_id: int) -> None:
    ...
    def _norm_domain(d: str | None) -> str:
        ...
        if not s:
            return ""
        return normalize_domain_root(s)
    ...
    dq_res = await db.execute(
        text(
            "SELECT id, replace(lower(domain), 'www.', '') AS domain, url, keyword "
            "FROM domains_queue "
    ...
    # 3) Fetch supplier cards for these domains
    #    Use a broader SQL match that handles protocol prefixes and subdomains
    #    in supplier_domains (e.g. 'https://mc.ru' or 'inoxpoint.ru' for 'spb.inoxpoint.ru')
    root_domains: list[str] = list(dict.fromkeys([nd for nd in uniq_by_domain.keys() if nd]))

    suppliers_res = await db.execute(
        text(
            "SELECT DISTINCT ON (ms.id) ms.id, COALESCE(ms.name, ''), ms.email, ms.phone, "
            "COALESCE(sd.domain, ms.domain) AS matched_domain, ms.data_status "
            ...
            "AND regexp_replace(regexp_replace(lower(COALESCE(sd.domain, ms.domain, '')), "
            "  '^https?://', ''), '^www\\.', '') = ANY(:domains)"
        ),
    ...
    for sr in suppliers_rows:
        ...
        s_data_status = str(sr[5] or "").strip()
        # FAIL-3 fix: Only show suppliers with data_status='complete' (Checko verified)
        if s_data_status != "complete":
            continue
