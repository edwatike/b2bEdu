...
async def list_parsing_runs_endpoint(
    ...
    # Batch: get domain counts for all runs in ONE query (fixes N+1)
    ...
    if run_ids:
        try:
            cnt_res = await db.execute(
                text(
                    "SELECT parsing_run_id, COUNT(*) "
                    "FROM domains_queue "
                    "WHERE parsing_run_id = ANY(:ids) "
                    "GROUP BY parsing_run_id"
                ),
                {"ids": run_ids},
            )
            for row in cnt_res.fetchall():
                domain_counts[str(row[0])] = int(row[1])
        except Exception:
            logger.warning("Batch domain count failed", exc_info=True)
    ...
    for run in runs:
        try:
            ...
            # 2) Fallback: request.raw_keys_json
            ...
            results_count = domain_counts.get(run.run_id) or None
