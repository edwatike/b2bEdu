...
class EnrichDomainResponse(BaseModel):
    ...
    error: str | None = None
    sourceUrls: list[str] = []
    extractionLog: list[dict] = []
    supplierType: str | None = None
...
async def list_pending_domains_endpoint(
    ...
    base_cte = (
        ...
        "), "
        "confirmed_suppliers AS ("
        f"  SELECT DISTINCT {supplier_expr} AS d FROM moderator_suppliers s "
        "  WHERE s.domain IS NOT NULL AND COALESCE(s.data_status, '') NOT IN ('requires_moderation', '') "
        "  UNION "
        f"  SELECT DISTINCT {supplier_domains_expr} AS d FROM supplier_domains sd "
        ...
        "    AND d NOT IN (SELECT d FROM confirmed_suppliers)"
        f"{search_filter}"
        ") "
    )
...
async def enrich_pending_domain(
    ...
    try:
        ...
        extraction_log = list(result.get("extractionLog") or [])
        inn_val = result.get("inn")
        emails_val = list(result.get("emails") or [])

        # Build reason explanation
        reason_parts = []
        if not inn_val:
            reason_parts.append("ИНН не найден")
        if not emails_val:
            reason_parts.append("Email не найден")
        if result.get("error"):
            reason_parts.append(f"Ошибка парсера: {result.get('error')}")
        ...
        try:
            ...
            return EnrichDomainResponse(
                ...
                reason="Ошибка сохранения поставщика",
            )

        data_status = result.get("dataStatus") or ("requires_moderation" if not inn_val else ("complete" if inn_val and emails_val else "needs_checko"))
        supplier_type = "supplier" if (inn_val and emails_val) else None

        reason_str = "; ".join(reason_parts) if reason_parts else "Все данные найдены"
