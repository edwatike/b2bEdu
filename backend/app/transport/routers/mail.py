...
async def fetch_yandex_folders_status(
    request: IMAPFoldersRequest,
...
async def send_yandex_email_smtp(
    request: SMTPRequest,
    current_user: dict = Depends(get_current_user),
):
...
async def fetch_yandex_emails_imap(
    request: IMAPRequest,
    current_user: dict = Depends(get_current_user),
):
...
async def fetch_yandex_email_imap_message(
    message_id: str,
    request: IMAPMessageRequest,
    current_user: dict = Depends(get_current_user),
):
    require_moderator(current_user)
    def _fetch_sync() -> dict:
...
async def unspam_yandex_email_imap_message(
    message_id: str,
    request: IMAPMoveRequest,
    current_user: dict = Depends(get_current_user),
):
    require_moderator(current_user)
    def _move_sync() -> dict:
