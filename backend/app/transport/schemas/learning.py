...
class LearnManualInnRequestDTO(BaseModel):
    ...
    sourceUrl: Optional[str] = Field(None, description="URL страницы, где найден ИНН")
    # Preferred multi-URL learning
    sourceUrls: Optional[List[str]] = Field(None, description="Список URL, где найден ИНН (для обучения паттернам)")
