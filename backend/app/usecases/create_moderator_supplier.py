...
async def execute(db: AsyncSession, supplier_data: dict):
    ...
    # Enforce project rule strictly: do not create supplier without INN + email
    inn_val = str(supplier_data.get("inn") or "").strip()
    email_val = str(supplier_data.get("email") or "").strip()
    if not (inn_val and email_val):
        raise ValueError("ИНН и Email обязательны для создания поставщика")

    supplier_data["type"] = "supplier"
    # Keep existing data_status if provided, otherwise default to complete
    supplier_data["data_status"] = supplier_data.get("data_status") or "complete"
