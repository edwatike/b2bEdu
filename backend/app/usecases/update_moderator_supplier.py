...
async def execute(db: AsyncSession, supplier_id: int, supplier_data: dict):
    ...
    # Enforce project rule strictly: supplier only when INN and Email exist.
    # Note: on update we may not get both fields in payload, so we must look at DB state too.
    repo = ModeratorSupplierRepository(db)
    current = await repo.get_by_id(supplier_id)
    if current is None:
        logger.warning(f"update_moderator_supplier.execute: supplier_id={supplier_id} not found")
        return None

    inn_val = str(supplier_data.get("inn") if "inn" in supplier_data else (getattr(current, "inn", None) or "")).strip()
    email_val = str(supplier_data.get("email") if "email" in supplier_data else (getattr(current, "email", None) or "")).strip()

    if not (inn_val and email_val):
        supplier_data["type"] = "candidate"
        supplier_data["data_status"] = "requires_moderation"
    else:
        supplier_data["type"] = "supplier"
        # If data_status is not explicitly provided, keep current or default to complete
        supplier_data["data_status"] = supplier_data.get("data_status") or getattr(current, "data_status", None) or "complete"
