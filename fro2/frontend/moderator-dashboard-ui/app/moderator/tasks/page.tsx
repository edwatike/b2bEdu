...
function ModeratorTasksPage() {
  ...
  const filteredAndSortedTasks = useMemo(() => {
    ...
    if (statusFilter !== "all") {
      filtered = filtered.filter(task => 
        (task.status || "new").toLowerCase() === statusFilter.toLowerCase()
      )
    }
  ...
  const stats = useMemo(() => {
    ...
    const running = tasks.filter(t => (t.status || "").toLowerCase() === "running" || (t.status || "").toLowerCase() === "processing").length
    const failed = tasks.filter(t => (t.status || "").toLowerCase() === "failed" || (t.status || "").toLowerCase() === "error").length
    const newTasks = tasks.filter(t => (t.status || "").toLowerCase() === "new").length
        ...
        <div className="mb-8">
          <div className="flex items-start justify-between gap-4 mb-6">
            <div>
              ...
              <p className="text-gray-600">Заявки, отправленные в работу: задача + все запуски парсинга по ключам.</p>
            </div>
            <Button 
              onClick={() => void load()} 
              disabled={isLoading}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="text-sm text-gray-600">
                        Задача #{t.id} · request #{t.request_id}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-4 mb-4 text-sm text-gray-600">
                    <div className="flex items-center gap-1">
                      <span className="font-medium">Источник:</span> {t.source}
                    </div>
                    <div className="flex items-center gap-1">
                    ...
                    <div className="mb-4">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-sm font-medium text-gray-700">
                          Прогресс запусков ({t.parsing_runs.filter(r => (r.status || "").toLowerCase() === "completed").length}/{t.parsing_runs.length})
                        </span>
                        <span className="text-xs text-gray-500">
                          {Math.round((t.parsing_runs.filter(r => (r.status || "").toLowerCase() === "completed").length / t.parsing_runs.length) * 100)}%
                        </span>
                      </div>
                      <Progress 
                        value={(t.parsing_runs.filter(r => (r.status || "").toLowerCase() === "completed").length / t.parsing_runs.length) * 100}
                        className="h-2"
                      />
                    </div>
                  )}
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    {t.parsing_runs && t.parsing_runs.length > 0 && (
                      <div className="space-y-2">
                        {t.parsing_runs.slice(0, 3).map((r) => (
