...
export function SupplierFinancials({ supplier }: SupplierFinancialsProps) {
  ...
  const latestCheckoYear = (() => {
    if (!checkoData?._finances) return null
    const years = Object.keys(checkoData._finances).filter(Boolean)
    if (!years.length) return null
    return years.sort((a, b) => Number(a) - Number(b)).pop() || null
  })()
  const derivedFinanceYear = supplier.financeYear ?? (latestCheckoYear ? Number(latestCheckoYear) : null)
  const currentYearData = derivedFinanceYear && checkoData?._finances?.[derivedFinanceYear.toString()]
  const derivedRevenue = toNumber(supplier.revenue ?? (currentYearData as any)?.["2110"] ?? null)
  const derivedProfit = toNumber(supplier.profit ?? (currentYearData as any)?.["2400"] ?? null)
  const derivedAuthorizedCapital = toNumber(supplier.authorizedCapital ?? checkoData?.УстКап?.Сумма ?? null)

  const [selectedYear, setSelectedYear] = useState(
    derivedFinanceYear?.toString() || new Date().getFullYear().toString()
  )
  ...
  const revenueChange = derivedRevenue ? ((derivedRevenue / 10000000) * 15).toFixed(1) : null
  const profitMargin = derivedRevenue && derivedProfit 
    ? ((derivedProfit / derivedRevenue) * 100).toFixed(1) 
    : null
        ...
        >
          <Card className="bg-gradient-to-br from-purple-50 to-white border-purple-200">
            <CardContent className="p-6">
              ...
              <p className="text-2xl font-bold text-neutral-900">
                {formatCurrency(derivedAuthorizedCapital)}
              </p>
        ...
        <Card>
          ...
          <CardContent>
            <div className="h-48 flex items-end justify-between gap-2">
              ...
              {generateMockData(derivedRevenue).map((value, index) => {
                const height = (value / Math.max(...generateMockData(derivedRevenue))) * 100
                return (
                  <motion.div
                    key={index}
